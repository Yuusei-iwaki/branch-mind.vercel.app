...
import { Copy, Check, Loader2, GitBranch, ChevronUp, ChevronDown, CheckCircle2 } from 'lucide-react';
...
export default function BranchGenerator() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResults(data.branchNames);
        } catch (err) {
            console.error(err);
            setResults([]);
            alert('ブランチ名の生成に失敗しました。時間をおいて再試行してください。');
        } finally {
            setLoading(false);
        }
    };
